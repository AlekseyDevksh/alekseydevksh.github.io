...
(function($) { "use strict";
	...
	function fillBars() {
		$('.bar').each(function() {
			 ...
			 bar.find('.progress-line').css('width', percent + '%' ).html('<span>'+percent+'</span>');
			});
	}
	// ------------------------------
	...
	function setActivePage() {
		...
		if(path == "") {  // if hash tag doesnt exists - go to first page
			var firstPage = $('.header__menu li').first().find('a').attr('href');
	...
	function setCurrentMenuItem() {
		var activePageId = $('.page-layout.page-current').attr('id');
		// set default nav menu
		$('.header__menu a[href$=' + activePageId +']').parent().addClass('current_page_item').siblings().removeClass('current_page_item');
	}
	// ------------------------------
	...
	var PageTransitions = (function() {
		...
		var $main = $( '.site-main' ),
			$pages = $main.children( '.page-layout' ),
			$menuLinks = $('.header__menu a'),
			animcursor = 1,
			endCurrPage = false,
			endNextPage = false,
			animEndEventNames = {
